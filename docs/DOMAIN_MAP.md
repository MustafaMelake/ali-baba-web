...
No models of its own — aggregates over `Order`, `OrderItem`, `User`, `Product`, `Branch`. Governing rule: **revenue/volume never counts `CANCELLED`** (`status: { not: CANCELLED } }`) — note this means **PENDING orders count toward revenue** (unconfirmed money; confirm as a business rule).

### 12.3 File Boundaries
`src/lib/actions/dashboard.ts` (`getDashboardStats`, `getOrders`, `ORDERS_PAGE_SIZE = 50`); `src/lib/actions/analytics.ts` (`getAnalytics`); pages `src/app/admin/page.tsx`, `src/app/admin/analytics/page.tsx` (both `force-dynamic`); charts `RevenueChart.tsx`, `analytics/{BranchSalesChart,PeakHoursChart}.tsx`; `src/lib/analytics-palette.ts` (stable per-branch colors by name-sorted index).

### 12.4 Step-by-Step Data Flow
**`getDashboardStats`:** scope → `branchWhere` → **12 parallel queries** in one `Promise.all` (all-time / last-30 / prev-30 revenue aggregates, today/yesterday order counts, product + customer counts (store-wide by design — no branchId on those models), 5 recent orders, 30-day chart rows, branch-name lookup) → JS buckets chart rows into 30 local calendar days.

**`getOrders`** *(paginated in commit `726134d`)*: scope → page clamped server-side (NaN/floats/negatives → 1) → order-number search via raw ``SELECT id FROM "Order" WHERE "orderNumber"::text ILIKE ${%q%}`` (an Int column can't `contains`; candidate ids may span branches but `branchWhere` is ANDed at the top level, so no cross-branch leak) → `findMany` with `skip/take` ‖ `groupBy(status)` for tab counters → pagination metadata (`total` = active tab's counter — **zero extra queries**; `hasMore = page × 50 < total`) → serialized `AdminOrderView[]` with VAT as the residual.

**`getAnalytics`** (ADMIN-only): active branches drive every series (zero-order branches still chart) → four parallel datasets: all-time + current-month groupBys; peak hours via raw SQL — `EXTRACT(HOUR FROM "createdAt" AT TIME ZONE 'UTC' AT TIME ZONE 'Africa/Cairo')::int`, `COUNT(*)::int` (casts avoid BigInt serialization), **30-day window** (parameter-bound) so stale seed data can't skew it; top products via `OrderItem ⋈ Order` rollup (top-3 per branch sliced in JS) → axis trimmed to the active trading window (fallback 8 AM–11 PM) → "Star of the Month" with revenue share.

### 12.5 Intersections & Blast Radius
RBAC (every query is scope-filtered — a new metric that forgets `branchWhere` is a cross-branch data leak), Orders (substrate; the VAT-residual and not-CANCELLED conventions), Branch (series identity/colors). The `AdminOrderFilters`/`AdminOrdersPagination` pair share the URL-param contract (`status`, `query`, `page`; filter changes delete `page`) — changing param names breaks both plus the page's parsers.

### 12.6 Technical Debt & Vestigial Code
- **Timezone asymmetry:** analytics buckets hours in `Africa/Cairo` explicitly; the dashboard's `startOfDay`/`dayKey` use the **server's local timezone** — "today's orders" shifts with deployment region. Candidate: pin both to the store TZ.
- Offset (`skip`) pagination is fine at current scale; cursor pagination (`createdAt`+`id`) is the upgrade path at tens of thousands of rows per filter.
- Revenue-counts-PENDING (see 12.2) — business-rule confirmation pending.
